      ...
      <Transition name="fade" mode="out-in">
        <div v-if="currentTab === 'overview'" class="tab-pane">
          <div class="kpi-grid">
            ...
            <div class="kpi-card glass-panel">
              <span class="kpi-label">TOTAL COGS / HPP</span>
              <h3 class="kpi-value text-danger">Rp {{ totalCOGS.toLocaleString() }}</h3>
              <TrendingUpIcon class="kpi-icon icon-danger" />
            </div>
            <div class="kpi-card glass-panel">
            ...
            <div class="kpi-card glass-panel">
              ...
              <h3 class="kpi-value">{{ grossMargin.toFixed(1) }}%</h3>
              <PercentIcon class="kpi-icon icon-primary" />
            </div>
            <div class="kpi-card glass-panel">
              <span class="kpi-label">PENDING REVENUE</span>
              <h3 class="kpi-value text-warning">Rp {{ pendingRevenue.toLocaleString() }}</h3>
              <ClockIcon class="kpi-icon icon-warning" />
            </div>
            <div class="kpi-card glass-panel">
              <span class="kpi-label">VOIDED AMOUNT</span>
              <h3 class="kpi-value text-dim">Rp {{ voidedAmount.toLocaleString() }}</h3>
              <BanIcon class="kpi-icon icon-dim" />
            </div>
          </div>
              ...
              <tbody>
                <tr v-for="pn in productPnL" :key="pn.label">
                  <td class="product-name">
                     <span class="dot" :class="pn.label === 'Hero Size' ? 'dot-hero' : pn.label === 'Standard Size' ? 'dot-standard' : 'dot-salted'"></span>
                     {{ pn.label }}
                  </td>
                  <td class="text-right">{{ pn.qty }} Pack</td>
                  <td class="text-right text-success">Rp {{ pn.rev.toLocaleString() }}</td>
                  <td class="text-right text-danger">Rp {{ pn.cogs.toLocaleString() }}</td>
                  <td class="text-right text-primary font-bold">Rp {{ pn.gp.toLocaleString() }}</td>
                  <td class="text-right" :class="getMarginClass(pn.margin)">{{ pn.margin.toFixed(1) }}%</td>
                </tr>
        ...
        <div v-if="currentTab === 'profitability'" class="tab-pane">
          <div class="profitability-grid">
            ...
            <div class="product-cards-container">
              <div v-for="pn in productPnL" :key="pn.label" class="prof-card glass-panel" :class="{'hero-border': pn.label==='Hero Size', 'std-border': pn.label==='Standard Size', 'asin-border': pn.label==='Telur Asin'}">
...
async function fetchFinancialData() {
  ...
  try {
    ...
    // 0. Fetch all inventory items for lookup (lightweight, no join needed)
    ...
    // 1. Fetch Sales with customers + nested sale_items
    ...
    // Attach inventory info to each sale_item manually
    const enrichedSales = (salesData || []).map((s: any) => ({
      ...s,
      sale_items: (s.sale_items || []).map((item: any) => ({
        ...item,
        inventory: invMap[item.egg_type] || null
      }))
    }));
    allSales.value = enrichedSales;

    // 2. Fetch stock arrival logs for Cash Flow (Money Out) — NO join
    const { data: stockLogsData, error: logsError } = await supabase
      .from('stock_logs')
      .select('*')
      .eq('log_type', 'arrival')
      .gte('created_at', startDate)
    ...
    if (logsError) {
      console.error('Stock logs query failed:', logsError);
    }
    
    // Attach inventory info manually
    restockLogs.value = (stockLogsData || []).map((log: any) => ({
      ...log,
      inventory: invMap[log.egg_type] || null
    }));

    // 3. Fetch Pending Receivables (with customer info)
    ...
    if (recError) {
      console.error('Receivables query failed:', recError);
    }
    pendingReceivables.value = receivablesData || [];
...
// 2. Total COGS / HPP
// Prefers snapshotted unit_cost (if saved at sale time), otherwise falls back to live inventory cost_price
const totalCOGS = computed(() => {
  return allSales.value
    .filter(s => s.payment_status === 'paid')
    .reduce((sum, s) => {
      const items = s.sale_items || [];
      const itemsCost = items.reduce((itemSum: number, item: any) => {
        const cost = item.unit_cost || item.inventory?.cost_price || 0;
        return itemSum + (Number(cost) * item.quantity);
      }, 0);
...
const chartData = computed(() => {
  ...
  allSales.value.filter(s => s.payment_status === 'paid').forEach(s => {
    ...
    dailyData[dateStr].rev += Number(s.total_price);
    
    const items = s.sale_items || [];
    const itemsCost = items.reduce((curSum: number, item: any) => {
      const cost = item.unit_cost || item.inventory?.cost_price || 0;
      return curSum + (Number(cost) * item.quantity);
    }, 0);
...
const productPnL = computed(() => {
  const products: Record<string, { label: string, rev: number, cogs: number, qty: number }> = {
    hero_size: { label: 'Hero Size', rev: 0, cogs: 0, qty: 0 },
    standard_size: { label: 'Standard Size', rev: 0, cogs: 0, qty: 0 },
    salted_egg: { label: 'Telur Asin', rev: 0, cogs: 0, qty: 0 }
  };
  
  allSales.value.filter(s => s.payment_status === 'paid').forEach(s => {
    const items = s.sale_items || [];
    items.forEach((i: any) => {
      if (products[i.egg_type]) {
        const prod = products[i.egg_type]!;
        prod.qty += i.quantity;
        prod.rev += Number(i.subtotal || (i.unit_price * i.quantity));
        const itemCost = i.unit_cost || i.inventory?.cost_price || 0;
        prod.cogs += (Number(itemCost) * i.quantity);
      }
    });
  });
...
const radarChartData = computed(() => {
  ...
  const datasets = productPnL.value.map(p => {
    if (p.label === 'Hero Size') return getDataset(p, '#f59e0b', 'rgba(245, 158, 11, 0.2)');
    if (p.label === 'Standard Size') return getDataset(p, '#10b981', 'rgba(16, 185, 129, 0.2)');
